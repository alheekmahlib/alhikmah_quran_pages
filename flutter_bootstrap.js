...
// تكوين مخصّص لإجبار CanvasKit renderer داخل iframe.
// يحل مشكلة "الشاشة الرمادية" التي تحدث أحيانًا عند تحميل Flutter Web داخل
// iframe عندما يختار المحرّك skwasm تلقائيًا ويفشل في التهيئة.
//
// المرجع: https://docs.flutter.dev/platform-integration/web/initialization

_dartContext = {
  canvasKitBaseUrl: "canvaskit/", // تأكد من مسار CanvasKit
};

_flutter.loader.load({
  onEntrypointLoaded: async function (engineInitializer) {
    let appRunner = await engineInitializer.initializeEngine({
      renderer: "canvaskit", // إجبار CanvasKit (جودة أعلى للخطوط العربية)
      hostElement: document.querySelector("#flutter-view") ?? undefined,
    });
    await appRunner.runApp();

    // إشعار الموقع الأم بأن القرآن جاهز (عبر postMessage)
    window.parent?.postMessage({ type: "quran:ready" }, "*");
  },
});
